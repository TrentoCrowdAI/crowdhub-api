    ...
    if (block.type === 'do') {
      ...
      const waitBlock = {
        ...
        parameters: {
          ...
          blockingContextId: block.parameters.blockingContextId
        },
        children: block.children,
        parents: [block]
      };
      // we update the ref of the children' parent so that they point to the newly created do-wait block.
      for (let child of waitBlock.children) {
        const blockIdx = child.parents.indexOf(block.id);
        child.parents = child.parents.splice(blockIdx, 1);
        child.parents.push(waitBlock);
      }
...
const getResult = async blocks => {
  ...
  for (let block of blocks) {
    while (block.promise === undefined) sleep(100);

    await block.promise;
    if (block.children.length === 0) results.push(block.result);
  }
