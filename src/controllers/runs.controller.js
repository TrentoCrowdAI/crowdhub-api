...
const sendRunResultAsCsv = async (result, fileNameWithoutExtension, res) => {
  try {
    ...
    let csv = await json2csvAsync(result, { emptyFieldValue: '' });
...
const sendCsvAsAttachment = (csv, fileNameWithoutExtension, res) => {
  res
    ...
    .header(
      'Content-Disposition',
      `attachment; filename="${fileNameWithoutExtension}.csv"`
    )
    .status(200)
    .send(csv);
};
...
const sendRunResultAsJson = (result, fileNameWithoutExtension, res) => {
  res
    .header(
      'Content-Disposition',
      `inline; filename="${fileNameWithoutExtension}.json"`
    )
    .json(result);
};
